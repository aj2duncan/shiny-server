...
defaults = list("tail" = "lower", "lower_bound" = "open", 
                "upper_bound" = "open")
...
shinyServer(function(input, output){ 
  output$tail = renderUI({
    ...
    if (input$dist == "rbinom") {
      selectInput(inputId = "tail", label = "Find Area:", 
                  choices = c("Lower Tail" = "lower", "Upper Tail" = "upper", 
  ...
  output$lower_bound = renderUI({
    ...
    if (input$dist == "rbinom") {
      ...
      if (input$tail %in% c("both","middle")) {
        selectInput(inputId = "lower_bound", label = "Lower bound:",
                    choices = c("<" = "open", "\u2264" = "closed"), 
                    selected = "open")
      } else if (input$tail == "lower") {
        selectInput(inputId = "lower_bound", label = "Bound:",
                    choices = c("<" = "open", "\u2264" = "closed"),
                    selected = "open")
  ...
  output$upper_bound = renderUI({
    ...
    if (input$dist == "rbinom") {
      ...
      if (input$tail == "middle") {
        selectInput(inputId = "upper_bound", label = "Upper bound:",
                    choices = c("<" = "open", "\u2264" = "closed"),
                    selected = "open")
  ...
  output$sd = renderUI({
    if (input$dist == "rnorm") {
      sliderInput("sd", "Standard deviation", value = 1, min = 0.1, max = 30,
                  step = 0.1)
    }
  })
  ...
  output$n = renderUI({
    if (input$dist == "rbinom") {
      sliderInput("n", "n", value = 10, min = 1, max = 250, step = 1)
    }
  })

  output$p = renderUI({
    if (input$dist == "rbinom") {
      sliderInput("p", "p", value = 0.5, min = 0, max = 1, step = .01)
    }
  })
  ...
  output$plot = renderPlot({ 
    ...
    if (error) {
      plot(0, 0, type = 'n', axes = FALSE, xlab = "", ylab = "", 
           mar = c(1, 1, 1, 1))
      text(0, 0, "Error: Lower bound greater than upper bound.", col = "red",
           cex = 2)
    } else {
      if (input$dist == "rnorm") {
        M = NULL
        ...
        plot(0, 0, type = 'n', xlim = c(-0.5, input$n + 0.5), 
             ylim = c(0, max(d)), xlab = "", ylab = "", axes = FALSE)
        axis(1, cex.axis = 1.5)
        axis(2, cex.axis = 1.5)
        ...
        for (k in 1:length(d)) {
            ...
            if (input$tail == "lower") {
              ...
              if (input$lower_bound == "open" & input$upper_bound == "open" & 
                  (k - 1 <  L | k - 1 >  U)) col = "#569BBD"
              if (input$lower_bound == "open" & input$upper_bound == "closed" & 
                  (k - 1 <  L | k - 1 >= U)) col = "#569BBD"
              if (input$lower_bound == "closed" & input$upper_bound == "open" & 
                  (k - 1 <= L | k - 1 >  U)) col = "#569BBD"
              if (input$lower_bound == "closed" & input$upper_bound == "closed" & 
                  (k - 1 <= L | k - 1 >= U)) col = "#569BBD"
            } else if (input$tail == "middle") {
              if (input$lower_bound == "open" & input$upper_bound == "open" & 
                  k - 1 > L & k - 1 < U) col = "#569BBD"
            ...
            p = matrix(c(-1.5 + k, 0, -0.5 + k, 0, -0.5 + k, d[k], -1.5 + k, 
                         d[k], -1.5 + k, 0), ncol = 2, byrow = TRUE)
            polygon(p, col = col)
        }
      }
    }
  ...
  output$area = renderText({
    ...
    if (input$dist == "rnorm") {
      ...
      if (input$tail == "equal") {
        ...
        if (input$tail %in% c("lower","both") & input$lower_bound == "open") {
          L = L - 1
        }
        if (input$tail %in% c("upper") & input$lower_bound == "closed") {
          L = L - 1
        }
        if (input$tail %in% c("middle") & input$lower_bound == "closed") {
          L = L - 1
        }
        ...
        if (input$tail %in% c("both","middle")) {
          ...
          if (input$tail == "both" & input$upper_bound == "closed") {
            U = U - 1
          } 
          if (input$tail == "middle" & input$upper_bound == "open") {
            U = U - 1
          }
        } 
      }
